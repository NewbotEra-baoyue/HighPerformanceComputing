...
class SMILESDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        x = torch.tensor(tokens[:-1], dtype=torch.long)  # 输入（去掉最后一个 token）
        y = torch.tensor(tokens[1:], dtype=torch.long)   # 标签（右移一位）
        return x, y
...
def generate(model, start_fragment, char2idx, idx2char, device,
    ...
    with torch.no_grad():
        for _ in range(max_new_tokens):
            ...
            logits = model(input_ids, mask)[:, -1, :]  # 取最后一个位置的 logits
            # 温度采样
            if temperature != 1.0:
...
def main():
    ...
    if args.mode == 'train':
        ...
        # 加载模型
        # checkpoint = torch.load(args.model_path, map_location=device) # PyTorch 2.6以前版本
